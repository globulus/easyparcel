...
public class SupportedTypes {
  ...
  public static CodeGenInfo getCodeGenInfo(VariableElement element, Elements elements,
    ...
    if (element.asType().getKind() == TypeKind.ARRAY) {
      ...
      ProcessorLog.error(element, "Unsuppored type %s as Array type for field %s. ",
    ...
    // Unsupported type
    ProcessorLog.error(element, "Unsuppored type %s for field %s. ",
        element.asType().toString(), element.getSimpleName());
  ...
  public static TypeMirror hasGenericsTypeArgumentOf(Element element, String typeToCheck,
    ...
    if (element.asType().getKind() != TypeKind.DECLARED
      ...
      ProcessorLog.error(element, "The field %s in %s doesn't have generic type arguments!",
    ...
    if (typeArguments.size() > 1) {
      ProcessorLog.error(element, "The field %s in %s has more than 1 generic type argument!",
          element.getSimpleName(), element.asType().toString());
    }
